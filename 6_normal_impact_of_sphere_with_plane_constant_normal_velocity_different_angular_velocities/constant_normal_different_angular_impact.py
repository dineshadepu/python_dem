...
def run():
    ...
    wz_values = np.linspace(0.1, 12, 30)
    vn = 0.2
    ang_vel_dem = []
    # create the particle
    for wz in wz_values:
        ...
        particle_u_vel.append(particle_1.u)
        particle_v_vel.append(particle_1.v)
        # print("wz is", particle_1.wz)

    # convert list into numpy arrays
    ang_vel_dem = np.asarray(ang_vel_dem)
    initial_tangential_velocity = wz_values * particle_1.radius
    initial_incident_angle = initial_tangential_velocity / vn

    ang_vel_dem = np.asarray(ang_vel_dem)
    particle_u_vel = np.asarray(particle_u_vel) + ang_vel_dem * particle_1.radius
    particle_v_vel = np.asarray(particle_v_vel)
    final_recoil_angle = ang_vel_dem * particle_1.radius / particle_v_vel
    ...
    data_initial_vs_final_dem_chung = np.loadtxt(
        os.path.join(
            directory,
            'dem_al_alloy_chung.csv'),
        delimiter=','
    )
    initial_dem_chung, final_dem_chung = data_initial_vs_final_dem_chung[:, 0], data_initial_vs_final_dem_chung[:, 1]
    plt.plot(initial_dem_chung, final_dem_chung, "^-", label='DEM Chung')

    plt.plot(initial_incident_angle, final_recoil_angle , "*-", label='DEM')
    # plt.scatter(cors, cor_dem, label='DEM')
    # plt.xlabel('Vt/(fric x Vn)')
    # plt.ylabel('r ang_vel / (fric x Vn)')
    plt.legend()
    fig = os.path.join(directory, "incident_vs_recoil.pdf")
