...
def run():
    ...
    for _theta in thetas:
        ...
        # wall.nu = nu
        # wall.G = G
        particle_1.u = velocity * np.sin(_theta)
        particle_1.v = -velocity * np.cos(_theta)
        dt = 1e-8
        _t = 0
        tf = 1000 * 1e-6
        gx = 0.
    ...
    # print(path)
    directory = os.path.dirname(path)
    data_theta_wz_analytical = np.loadtxt(
        os.path.join(
            directory,
            'sphere_plane_oblique_impact_experimental.csv'),
        delimiter=','
    )
    theta_analytical, wz_analytical = data_theta_wz_analytical[:, 0], data_theta_wz_analytical[:, 1]
    plt.scatter(theta_analytical, wz_analytical, label='Experimental')

    data_theta_wz_chung = np.loadtxt(
        os.path.join(
            directory,
            'sphere_plane_oblique_impact_dem_chung.csv'),
        delimiter=','
    )
    theta_chung, wz_chung = data_theta_wz_chung[:, 0], data_theta_wz_chung[:, 1]
    plt.plot(theta_chung, wz_chung, "^--", label='DEM Chung')
    ...
    plt.legend()
    fig = os.path.join(directory, "wz_vs_theta.pdf")
